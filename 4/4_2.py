...
for number in numbers:
    ...
    for board in boards:
        # Each board must be checked for each number. A new number my eliminate
        # more than one board
        try:
            # Test each line
            ...
            # Strip the chosen board from the boards list
            boards.remove(chosenBoard)
        
    # and go round again only if there is more than one board left
    if len(boards) == 0:
        break
