def fuel(moveTo, positions):
    fuel = 0
    for crab in positions:
        distance = abs(moveTo - crab)
        # Fuel is now a function of distance described as ((dist**2)+dist)/2
...
while not done:
    ...
    newFuel = fuel(moveDown, positions)
    if newFuel < bestFuel:
        # Beaten previous best
        bestFuel = newFuel
        test = moveDown
